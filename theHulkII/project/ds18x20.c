...
uint8_t DS18X20_meas_to_cel( uint8_t fc,
								uint8_t *sp,
								uint8_t* subzero,
								uint8_t* cel,
...
uint16_t DS18X20_temp_to_decicel( uint8_t subzero, 
									uint8_t cel, 
									uint8_t cel_frac_bits)
{
	uint16_t h;
...
int8_t DS18X20_temp_cmp( uint8_t subzero1, 
							uint16_t cel1, 
							uint8_t subzero2, 
							uint16_t cel2 )
{
...
void DS18X20_find_sensor( uint8_t *diff, uint8_t id[] )
{
	...
	for (;;) {
		...
		*diff = ow_rom_search( *diff, &id[ 0 ] );
		...
		if ( DS18B20_ID == id[0] || DS18S20_ID == id[0] ) return;
...
#ifdef DS18X20_VERBOSE
		uart0Puts( "DS18X20_start_meas: Short Circuit !\r\n" );
#endif
...
uint8_t DS18X20_read_meas( uint8_t id[], 
							uint8_t *subzero, 
							uint8_t *cel, 
							uint8_t *cel_frac_bits )
{
...
//
// reads temperature (scratchpad) of sensor with rom-code id
// and return it time 100
//

int DS18X20_getTemp100( uint8_t id[] )
{
	uint8_t i;
	uint8_t sp[ DS18X20_SP_SIZE ];
	...
	// ( 100 * ( 16 - sp[ 6 ] ) / 16 ) ); //
	return ( 50 * ( ( sp[0] + ( sp[ 1 ] << 8 ) ) ) - 25 +   ( ( 1600 - ( 100 * sp[ 6 ] ) ) / 16 ) );
}
